...
import { closingHour, openingHour, slotDurationOptions } from "@/lib/time-slots";
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-md border border-stone-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 grid gap-2">
                {[
                  `Operating hours: ${String(openingHour).padStart(2, "0")}:00 - ${String(closingHour).padStart(2, "0")}:00`,
                  `Duration options: ${slotDurationOptions
                    .map((duration) => `${duration} hour${duration > 1 ? "s" : ""}`)
                    .join(" or ")}`,
                  "Maximum reservation length: 2 hours per slot",
                ].map((slot) => (
                  <div
                    key={slot}
